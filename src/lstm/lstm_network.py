...
class DeepARLSTMNetwork(mx.gluon.HybridBlock):
    @validated()
    def __init__(
        self,
        num_layers: int,
        ...
        **kwargs,
        ...
        with self.name_scope():
            # === OLD DEEP_AR CODE ===
            # distr_output.get_args_proj()
            # === OLD DEEP_AR CODE ===

            self.proj_distr_args = mx.gluon.nn.Dense(1, flatten=False)
...
class DeepARLSTMTrainingNetwork(DeepARLSTMNetwork):
    def distribution(
        self,
        ...
        return F.broadcast_mul(distr_args, scale.expand_dims(axis=-1).repeat(axis=1, repeats=len(distr_args[0])))
    ...
    def hybrid_forward(
        ...
        loss = self.mape_loss(F, distr, target)
...
class DeepARLSTMPredictionNetwork(DeepARLSTMNetwork):
    @validated()
    ...
    def sampling_decoder(
        ...
        for k in range(self.prediction_length):
            ...
            new_samples = distr_args.squeeze(axis=-1) * repeated_scale
